...
def _prepare(database_path: Path, *, owner: str = "TEST-USER", tag: str = "vtj-api"):
    """Registers, renders, and gates an experiment into `training` -- the
    training job wraps `evaluate_visual_experiment()`, which requires that
    state.
    """
    initialize_database()
...
def test_enqueue_training_job_processes_via_background_task_and_completes(
    ...
    with TestClient(app) as client:
        ...
        created = client.post(
            f"/api/v2/visual-experiments/{experiment.experiment_id}/training-jobs",
            json=TRAINING_JOB_BODY, headers=headers,
        )
        assert created.status_code == 202
        job_id = created.json()["data"]["job_id"]
        assert job_id.startswith("vtj_")
        detail = client.get(
            f"/api/v2/visual-experiments/{experiment.experiment_id}/training-jobs/{job_id}",
            headers=headers,
        )
    assert detail.status_code == 200
    body = detail.json()["data"]
    assert body["state"] == "completed"
    assert body["result"]["experiment"]["lifecycle_state"] in (
        "evaluated", "out_of_distribution", "insufficient_evidence",
    )
    assert body["result"]["evaluation"]["holdout_metrics"]["sample_count"] >= 0


def test_enqueue_training_job_is_idempotent_by_key(isolated_database: Path) -> None:
    experiment = _prepare(isolated_database)
    with TestClient(app) as client:
        headers = _headers(client)
        first = client.post(
...
def test_enqueue_training_job_missing_experiment_returns_404(isolated_database: Path) -> None:
    initialize_database()
    apply_migrations(isolated_database, application_version="0.3.0")
    with TestClient(app) as client:
        headers = _headers(client)
        response = client.post(
            "/api/v2/visual-experiments/does-not-exist/training-jobs",
            json=TRAINING_JOB_BODY, headers=headers,
        )
    assert response.status_code == 404


def test_training_job_detail_rejects_wrong_experiment_path(isolated_database: Path) -> None:
    experiment = _prepare(isolated_database)
    with TestClient(app) as client:
        headers = _headers(client)
        created = client.post(
        ...
        mismatched = client.get(
            f"/api/v2/visual-experiments/some-other-experiment/training-jobs/{job_id}",
            headers=headers,
        )
    assert mismatched.status_code == 404


def test_training_job_detail_missing_returns_404(isolated_database: Path) -> None:
    experiment = _prepare(isolated_database)
    with TestClient(app) as client:
        headers = _headers(client)
        response = client.get(
            f"/api/v2/visual-experiments/{experiment.experiment_id}/training-jobs/does-not-exist",
            headers=headers,
        )
    assert response.status_code == 404


def test_cancel_terminal_training_job_returns_409(isolated_database: Path) -> None:
    experiment = _prepare(isolated_database)
    with TestClient(app) as client:
        headers = _headers(client)
        created = client.post(
        ...
        job_id = created.json()["data"]["job_id"]
        cancel = client.post(
            f"/api/v2/visual-experiments/{experiment.experiment_id}/training-jobs/{job_id}/cancel",
            headers=headers,
        )
    assert cancel.status_code == 409


def test_training_job_endpoints_require_ownership(isolated_database: Path) -> None:
    experiment = _prepare(isolated_database, owner="OTHER-OWNER", tag="vtj-owner")
    with TestClient(app) as client:
...
def test_training_job_rejects_unsupported_architecture(isolated_database: Path) -> None:
    experiment = _prepare(isolated_database, tag="vtj-badarch")
    body = dict(TRAINING_JOB_BODY)
    body["architecture_id"] = "some_unsupported_architecture"
    with TestClient(app) as client:
        headers = _headers(client)
        created = client.post(
            f"/api/v2/visual-experiments/{experiment.experiment_id}/training-jobs",
