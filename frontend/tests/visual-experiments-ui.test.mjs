...
test("visual experiments panel stays research-only and registers only frozen configuration", async () => {
  ...
  // Training+evaluation job workflow: same restore/cancel wiring, gated on
  // the `training` lifecycle state, surfacing holdout accuracy/outcome.
  assert.match(source, /training-jobs`/);
  assert.match(source, /Modeli öyrət və qiymətləndir/);
  assert.match(source, /TRAINING_JOB_STORAGE_PREFIX/);
  assert.match(source, /experiment\.lifecycle_state !== "training"/);
  assert.match(source, /result\.evaluation\.outcome/);
