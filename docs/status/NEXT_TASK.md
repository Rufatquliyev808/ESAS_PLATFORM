...
Mərhələ: Phase 5 (Visual AI) YENİDƏN AKTİV PRİORİTETDİR — renderer+dataset+label+qeydiyyat API-si+frontend paneli+materializer v1+`registered → rendering` persistence+yerli content-addressed artifact store+async job/API resursu (rendering)+rendering job-un frontend workflow-a bağlanması+`rendering → training` müqaviləsi və təhlükəsizlik qapıları+training başlanğıcının atomikləşdirilməsi+deterministik training-input pipeline v1+deterministik CPU visual baseline trainer v1+evaluation/OOD-abstain + `training → evaluated`+accept/reject qərarı+statistik acceptance qapısı və multiple-testing reyestri+async job/API resursu (training+evaluation)+training/evaluation job-un frontend-ə bağlanması+async job/API resursu (statistik accept/reject qərarı)+statistik accept/reject qərarının frontend-ə bağlanması+**`accepted_for_shadow` eksperimentin SHADOW challenger-inə lineage-only bağlanması** (bu sessiyada tamamlandı — CANLI qərar generatoru DEYİL) hamısı hazırdır. Qalan: real Phase 9 canlı qərar generatoru (Phase 6-8 hələ dizayn mərhələsindədir — bu, lineage-dən fərqli, çox böyük ayrı iş), real ML kitabxanası/GPU seçimi. Sonra Phase 6 (Xəbər/fundamental, hələ 0%) tamamlanmalıdır ki, Phase 7-yə qayıtmaq mümkün olsun. Migration `0012`-`0022` YALNIZ test bazasındadır — real production bazaya tətbiq edilməyəcək (istifadəçinin açıq qərarı).
...
- **Phase 5 (Visual AI) — AKTİV PRİORİTET** (Phase 7 dayandırılandan
  ...
  paneli, **Deterministic Visual Dataset Materializer v1**
  (`visual_materializer.py`) VƏ **`registered → rendering` job/lifecycle
  persistence** (migration `0013`, `visual_dataset_repository.py`,
  `visual_experiment_repository.py`-də `start_rendering()`/
  `mark_rendering_failed()`, `strategies/visual_experiment_materialization.py`-də
  `render_visual_experiment()` orkestrasiyası) VƏ **yerli
  content-addressed artifact store** (`docs/architecture/ARTIFACT_STORE_CONTRACT.md`,
  `storage/artifact_store.py`, migration `0013`-ə `artifact_checksum`
  ...
  **deterministik training-input pipeline v1** (`visual_render.py`-də
  ...
  kimi saxlanması) VƏ **deterministik CPU visual baseline trainer v1**
  (`analysis/visual_baseline_trainer.py` — `pixel_centroid_baseline_v1`
  arxitekturası, `fit_baseline_model()` YALNIZ train-dən sabit sırada
  centroid hesablayır, `predict_validation()` nearest-centroid + score,
  yeni ML/GPU asılılığı yoxdur; yeni `strategies/visual_baseline_training.py`
  — eksperiment ARTIQ `training`-də olmalıdır, `holdout_samples` DƏRHAL
  atılır, model+log artefaktları content-addressed store-a, metadata
  migration `0016`-ya yazılır; eksperiment `training`-də qalır, `evaluated`
  ayrı addımdır) VƏ **evaluation/OOD-abstain + `training → evaluated`**
  (`analysis/visual_evaluation.py` — `compute_majority_baseline_accuracy()`
  (sadə müqayisəçi), `detect_out_of_distribution()` (holdout vs validation
  orta ən-yaxın-centroid məsafəsi), `build_evaluation_artifact()`
  (outcome: `insufficient_evidence` → `out_of_distribution` → `evaluated`
  prioritet sırası); migration `0017`; `visual_experiment_repository.py`-də
  `mark_evaluated`/`mark_out_of_distribution`/`mark_insufficient_evidence`;
  yeni `strategies/visual_experiment_evaluation.py` — HOLDOUT-un istifadə
  edildiyi ilk və yeganə yer) VƏ **accept/reject qərarı**
  (`analysis/visual_acceptance.py` — `decide_acceptance()`: holdout
  accuracy majority-baseline-i `MINIMUM_IMPROVEMENT_OVER_BASELINE` (v1
  hədd) qədər keçmirsə `rejected`, keçirsə `accepted_for_shadow`; migration
  `0018`; `visual_experiment_repository.py`-də `mark_accepted_for_shadow`/
  `mark_rejected`; yeni `strategies/visual_experiment_acceptance.py` —
  persisted evaluation-u oxuyur, yenidən hesablamır — SONRA statistik
  qapı ilə əvəz edildi, bax aşağı) VƏ **statistik acceptance qapısı və
  multiple-testing reyestri** (`analysis/visual_statistical_acceptance.py`
  — `compute_testing_family_id()`, dəqiq `one_sided_binomial_p_value()`,
  `wilson_score_interval()`, `bonferroni_corrected_alpha()`,
  `decide_statistical_acceptance()` (6 şərt); migration `0019`,
  `visual_testing_trial_repository.py` — `register_trial()` idempotent,
  `count_family_trials()`; `train_visual_baseline_model()` indi trial-ı
  NƏTİCƏDƏN ƏVVƏL qeydə alır; `decide_visual_experiment_acceptance()`
  tam yenidən qurşandırılıb — modeli/holdout-u TƏZƏ hesablayır (reproduksiya
  yoxlaması), family trial sayını axtarır, statistik qapını çağırır —
  köhnə sadə 5% yol artıq bu yoldan HEÇ VAXT çağırılmır) tamamlandı.
  ...
  endpoint) VƏ **training/evaluation job-un frontend-ə bağlanması**
  (`visual-experiments-panel.tsx`-də yeni `TrainingJobCell` — mövcud
  `RenderingJobCell` nümunəsini güzgüləyir, `lifecycle_state === "training"`
  olduqda göstərilir, tamamlandıqda evaluation outcome+holdout accuracy+
  majority-baseline göstərir) indi hamısı mövcuddur (yalnız test/scratch
  bazada — real production-a YOX; accept/reject qərarı SHADOW-a heç bir
  real bağlantı yaratmır, YALNIZ uyğunluğu qeyd edir; STATİSTİK accept/
  reject qərarının ÖZÜ hələ frontend-ə bağlanmayıb). Növbəti təbii
  addımlar: (1) real production bazaya migration `0012`-`0020` tətbiqi —
  ayrıca açıq təsdiq lazımdır; (2) statistik accept/reject qərarının
  frontend-ə bağlanması; (3) `accepted_for_shadow` eksperimentin real
  Phase 9 SHADOW sisteminə bağlanması; (4) real ML kitabxanası/GPU qərarı
  (daha mürəkkəb arxitekturalar üçün) — daha böyük, ayrıca qərar tələb
  edən addım. İstifadəçinin prioritet sırasına görə (Phase
  3→4→7→8→9→10)
